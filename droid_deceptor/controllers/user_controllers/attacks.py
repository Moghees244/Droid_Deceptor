...
class Attacks:
    ...
    def noob_attack(self):
        ...
        # Attack
        for i in range(len(adversarial_example[0])):
            if adversarial_example[0][i] == 0:
                # manipulate a feature in the test vector
                adversarial_example[0][i] = 1

                # Get the result and probabilities with the manipulated vector
                test_class, _ = self.model(adversarial_example)

                # Check if the classifier result changed
                if test_class != original_class:
                    self.results(adversarial_example)
                    self.perturbations(adversarial_example)
                    return

        # If no successful attack is found
        print("Attack Unsuccessful!")
    
    def simulated_annealing_attack(self):
        print("Simulated Annealing ATTACK")
        original_class = self.initial_data()
        adversarial_example = self.apk_vector.copy()
        _, probs = self.model(adversarial_example)
        # Create a copy of the APK vector for manipulation
        adversarial_example = self.apk_vector.copy()

        temp = 10
        bestHeuristic = probs[0][1]

        while temp > 0.001:
            # chnage a random feature
            random_feature = random.randrange(0, len(adversarial_example[0]))
            if adversarial_example[0][random_feature] == 0:
                adversarial_example[0][random_feature] = 1                

            if self.model(adversarial_example)[0] != original_class:
                print("Attack Successful!")
                self.results(adversarial_example)
                self.perturbations(adversarial_example)
                return adversarial_example

            tempHeuristic = self.model(adversarial_example)[1][0][1]
            ...
            if deltaE < 0:
                ...
                if probability > random.uniform(0, 1):
                    ...
                    adversarial_example[0][random_feature] = 0
            temp *= 0.9
